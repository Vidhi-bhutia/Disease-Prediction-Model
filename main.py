...
def predict_disease(symptoms):
    feature_array = np.zeros((1, 133))  # Set this to the correct number of features

    # Existing symptom indices mapping
    symptom_indices = {
        ...
        'fluid_overload': 117, 'blood_in_sputum': 118, 'prominent_veins_on_calf': 119,
    ...
    # Mark symptoms present by setting indices to 1
    for symptom in symptoms:
        if symptom in symptom_indices:
            feature_array[0][symptom_indices[symptom]] = 1

    # Predict the diseases
    predicted_probabilities = model.predict_proba(feature_array)
    predicted_classes = model.predict(feature_array)

    # Get disease names and probabilities
    diseases = label_encoder.inverse_transform(predicted_classes)
    probabilities = predicted_probabilities.max(axis=1) * 100

    # Display diseases with confidence levels
    disease_confidence = {diseases[i]: probabilities[i] for i in range(len(diseases))}
    return disease_confidence

# Example usage
if __name__ == "__main__":
    print("Enter symptoms (separate symptoms by commas): ")
    user_input = input().strip()
    user_symptoms = [symptom.strip() for symptom in user_input.split(',')]

    results = predict_disease(user_symptoms)
    
    print("Predicted diseases and confidence levels:")
